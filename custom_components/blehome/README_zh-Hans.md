...
### v2.0.0
- **广播名 TLED_XXXX**: 显示 MAC 地址后 4 位，方便区分多台设备
- **最新固件默认选中**: 选项页面自动选择最新固件文件
- **配网后自动跳转网关配置**: 首次配网自动引导添加网关
- **Mesh 定时扫描**: 网关定期扫描 Mesh 网络，新设备自动发现无需重启 HA
- **BLE 连接稳定性**: 修复竞态条件、优化断开重连、清理状态残留
- **代码注释全部英文化**: 统一使用英文注释
- **文案修正**: 去除"LED闪烁"错误描述
